...
class event_extraction:
    ...
    def run(self):
        self.createMapMask(self.mapType)
    ...
    def event_extraction(self):
        ...
        self.video_nFrame = self.vidInfo['nFrames']
        ...
        while True:# self.frameCount <10:  # Capture frame-by-frame
            ...
            if self.ret:
                ...
                if self.gazecentered:
                    self.loadImage_gazecentered(self.frame, self.frameCount,
                                                self.sampledgazexData_featureExtraction, self.sampledgazeyData_featureExtraction,
                                                self.subject,self.movieNum, scrGamFac=self.scrGamFac, maxlum = self.maxlum,eventExtraction = True)
                else:
                ...
                if self.frameCount >= np.linspace(0, self.video_nFrame, 11)[pb_value]:
                    ...
                    if self.useApp:
                        ...
                        label_percent.grid(column = 0, row = 4, columnspan = 2)

                        self.label_info.grid_forget()
                        self.button_featureextraction.grid_forget()
                ...
                if self.frameCount ==1:
                    print(f"final: {self.levelMeanMatPart.shape}")
    ...
    def overall_lum_extraction(self):
        if self.gazecentered:
            self.createMapMask(mapType = "square")
        ...
        while True:# self.frameCount <10:  # Capture frame-by-frame
            ...
            if self.ret:
                # for overall luminance extraction, we don't use gaze position
                # consider the gaze is always at the center
                # loadImage_gazecentered is used to take the part outside the video screen into account
                #for overall luminance extraction, square map is used

                self.loadImage_gazecentered(self.frame, self.frameCount,
                                            np.nan, np.nan,
                                            '', self.movieNum, scrGamFac=self.scrGamFac, maxlum = self.maxlum,eventExtraction = False)
                img_lum = self.img[:,:,0].mean()
                luminance_all.append(img_lum)
                #print(f"movie: {self.movieNum} frame: {self.frameCount} luminance: {img_lum}")
                # track the progress
                if self.frameCount >= np.linspace(0, self.video_nFrame, 11)[pb_value]:
        ...
        self.cap.release()
        luminance_movie = np.nanmean(luminance_all)
        print(f"movie: {self.movieNum} overall luminance: {luminance_movie}")
        return luminance_movie
    ...
    def loadImage_gazecentered(self, img, frameNum, sampledgazexData, sampledgazeyData, subject, movieNum, scrGamFac=2.2, rotDeg=0, maxlum = 212, eventExtraction=True, showImageProcResult=False):
        """
        ...
        if eventExtraction:
            ...
            mapType = "square"
            self.squareHeight = self.eyetracking_height
            self.squareWidth =  self.eyetracking_width
        if self.videoScreenSameRatio and self.videoStretched:
            ...
            screen = np.ones((int(self.eyetracking_height), int(self.eyetracking_width),np.shape(img)[2]),dtype = 'float32')
        ...
        if gazex<0 or gazey <0 or gazex > np.shape(img)[1]-1 or  gazey > np.shape(img)[0]-1:
            print(f'Note:{subject} movie {movieNum} frameNum {frameNum} is out of the screen (gazex:{gazex}; gazey:{gazey})')
        new_img = np.zeros((finalImgHeight, finalImgWidth, 3), dtype = "float32")

        if np.isfinite(gazex) and np.isfinite(gazey):
            # Desired crop size
            crop_h, crop_w = finalImgHeight, finalImgWidth
            half_h = crop_h // 2
            half_w = crop_w // 2

            # Calculate crop coords in big image
            y_start_old = gazey - half_h
            y_end_old   = gazey + half_h
            x_start_old = gazex - half_w
            x_end_old   = gazex + half_w

            # Corresponding coords in new_img
            y_start_new = max(0, -int(np.floor(y_start_old)))
            x_start_new = max(0, -int(np.floor(x_start_old)))
            y_end_new   = crop_h - max(0, int(np.ceil(y_end_old) - img.shape[0]))
            x_end_new   = crop_w - max(0, int(np.ceil(x_end_old) - img.shape[1]))

            # Clip old image coords (ensure they are within bounds)
            y_start_old = max(0, int(np.floor(y_start_old)))
            x_start_old = max(0, int(np.floor(x_start_old)))
            y_end_old   = min(img.shape[0], int(np.ceil(y_end_old)))
            x_end_old   = min(img.shape[1], int(np.ceil(x_end_old)))

            # Paste overlapping part
            y_new = y_end_new-y_start_new
            x_new = x_end_new-x_start_new
            y_old = y_end_old-y_start_old
            x_old = x_end_old-x_start_old
            diff_y = y_new-y_old
            diff_x = x_new-x_old
            if diff_y >0:
                y_end_new =y_end_new - (diff_y)
            else:
                y_end_old =y_end_old + (diff_y)
            if diff_x>0:
                x_end_new =x_end_new - (diff_x)
            else:
                x_end_old =x_end_old + (diff_x)
            #print(diff_y, diff_x)

            new_img[y_start_new:y_end_new, x_start_new:x_end_new] = \
                img[y_start_old:y_end_old, x_start_old:x_end_old]

            finalImg = new_img
            if self.countIm ==1:
                print(f"{subject} movie {movieNum}")
                print(f"original(screen): {img.shape}")
                print(f"final(map): {finalImg.shape}")
        ...
        if eventExtraction:
            # apply mask if it is circular (please don't try to add mapType == "square" in here )
            if mapType == "circular":
    ...
    def stimMatrixFunc(self):
        ...
        #print(circleDiameterCM, eyetracker_pixel_1cm, circleDiameterPixel)

        if self.nWeight == 24:
            annulusEccentricity_pix = list(np.around(np.array([109, 218, 336, 447]) * (halfCircleDiameterPixel/447)).astype(int))
            nWedgesPerAnnulus = [4, 6, 6, 8]
            tempRadii = np.hstack((0, annulusEccentricity_pix))

        elif self.nWeight == 44 or self.nWeight == 2:
            annulusEccentricity_pix = list(np.around(np.array([70, 149, 218, 336, 447]) * (halfCircleDiameterPixel/447)).astype(int))
            nWedgesPerAnnulus = [4, 8, 8, 12, 12]
            tempRadii = np.hstack((0, annulusEccentricity_pix))

        elif self.nWeight == 20:
            annulusEccentricity_pix = list(np.around(np.array([7.42, 11.810400000000001, 16.727648000000002, 22.23496576]) * (halfCircleDiameterPixel/22.23496576)).astype(int))
            nWedgesPerAnnulus = [6,6,8]
            tempRadii = annulusEccentricity_pix # the center part is not counted as wedges (gaze included)
...
##
        ...
        if self.nWeight == 2:
            ...
            weightLeft_regions = [int(x.replace("weight",'')) for x in left]
            weightRight_regions = [int(x.replace("weight",'')) for x in right]
            # left= 1, right = 2
            stimMatrix[np.isin(stimMatrix, weightLeft_regions)] = 100
            stimMatrix[np.isin(stimMatrix, weightRight_regions)] = 200
            stimMatrix[stimMatrix == 200] = int(2) #right = 1, left = 2
            stimMatrix[stimMatrix == 100] = int(1)
        # plt.figure()
        # plt.imshow(stimMatrix)
        # plt.show()
        ...
        return stimMatrix
    def createMapMask(self,mapType):
        if mapType == "circular":
    ...
    def meanPerMatPart(self, nVertMatPartsPerLevel, aspectRatio, tarMat):
        ...
        if self.nWeight == 6:
            templateRegion = np.array([[3,3,3,3,3,3,3,3],
                                        [3,3,2,2,2,2,3,3],
                                        [3,3,2,1,1,2,3,3],
                                        [6,6,5,4,4,5,6,6],
                                        [6,6,5,5,5,5,6,6],
            ...
            levelMeanMatPartRemap =[]
            for i in range(self.nWeight):
                regionIndex = int(i +1)
                meanRegion = self.levelMeanMatPart[templateRegion == regionIndex].mean()
                levelMeanMatPartRemap.append(meanRegion)
            self.levelMeanMatPart = np.array(levelMeanMatPartRemap)
